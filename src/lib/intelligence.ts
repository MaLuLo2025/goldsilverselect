import { IntelligenceItem } from "./types";

export const intelligenceItems: IntelligenceItem[] = [
  {
    slug: "mcalvany-ratio-opportunities",
    type: "video",
    speaker: "David McAlvany",
    title: "Gold & Silver: Turbulent Reversals & Ratio Opportunities",
    source: "McAlvany Weekly Commentary",
    date: "Feb 4, 2026",
    tag: "Specialist",
    url: "https://mcalvany.com/weekly-commentary/gold-silver-turbulent-reversals-ratio-opportunities/",
    description:
      "McAlvany covers the gold/silver bull market thesis, six years of silver supply deficit, and central bank net buying. Discusses when to start swapping silver to gold using ratio trading. The McAlvany Weekly Commentary has run for 18 years \u2014 one of the longest-running precious metals podcasts.",
  },
  {
    slug: "druckenmiller-hard-lessons",
    type: "video",
    speaker: "Stan Druckenmiller",
    title: "Hard Lessons: Invest, Then Investigate",
    source: "Morgan Stanley",
    date: "Jan 30, 2026",
    tag: "Macro",
    url: "https://www.morganstanley.com/insights/videos/hard-lessons/duquesne-stan-druckenmiller-iliana-bouzali",
    youtubeUrl: "https://www.youtube.com/watch?v=z_pk4eBDaLA",
    description:
      "Druckenmiller\u2019s Jan 30, 2026 conversation with Morgan Stanley covers his current portfolio positioning. He holds gold as a geopolitical hedge and is bearish on the dollar. Key quote: \u2018Foreigners are way, way overloaded in dollars.\u2019 Full interview covers Japan, Korea, copper, and AI.",
  },
  {
    slug: "dalio-safest-money",
    type: "research",
    speaker: "Ray Dalio",
    title: "Gold Is Still the Safest Money",
    source: "World Governments Summit / Yahoo Finance",
    date: "Feb 2026",
    tag: "Macro",
    url: "https://finance.yahoo.com/news/billionaire-ray-dalio-drops-blunt-173500432.html",
    description:
      "At the World Governments Summit in Dubai, Dalio called gold \u2018the safest money\u2019 and warned of an approaching \u2018capital war.\u2019 He recommends 5\u201315% portfolio allocation to gold given market fragility. His 2025 year-end review noted gold returned 65% in dollar terms, outperforming the S&P by 47%.",
  },
  {
    slug: "wgc-outlook-2026",
    type: "research",
    speaker: "World Gold Council",
    title: "Gold Outlook 2026: Push Ahead or Pull Back",
    source: "World Gold Council",
    date: "Dec 4, 2025",
    tag: "Institutional",
    url: "https://www.gold.org/goldhub/research/gold-outlook-2026",
    description:
      "Comprehensive institutional analysis of gold\u2019s macro drivers. Projects 5\u201315% upside depending on the severity of economic slowdown and pace of rate cuts. Covers central bank demand, de-dollarization, ETF inflows, and geopolitical risk scenarios.",
  },
  {
    slug: "rick-rule-trust-collapse",
    type: "video",
    speaker: "Rick Rule",
    title: "The Trust Collapse Is Here: Gold, Silver, Copper, Uranium and the Real Asset Reset",
    source: "Kitco News",
    date: "Aug 2025",
    tag: "Specialist",
    url: "https://www.kitco.com/news/tag/rick-rule",
    description:
      "Legendary resource investor Rick Rule dissects the gold miner breakout, looming silver supply crunch, and why the \u2018bubble in trust\u2019 that fueled markets for four decades is now eroding. Covers why gold miners\u2019 earnings create an M&A setup, and why silver could squeeze.",
  },
  {
    slug: "jpmorgan-gold-forecast",
    type: "research",
    speaker: "J.P. Morgan",
    title: "Gold Price Forecast: Toward $5,000 by Year-End 2026",
    source: "J.P. Morgan Global Research",
    date: "Jan 2026",
    tag: "Institutional",
    url: "https://www.jpmorgan.com/insights/global-research/commodities/gold-prices",
    description:
      "JPMorgan\u2019s commodities team projects central bank and investor demand averaging 585 tonnes per quarter in 2026. Gold remains their highest-conviction long. They forecast ~250 tonnes of ETF inflows and bar/coin demand above 1,200 tonnes annually.",
  },
];

export function getIntelligenceBySlug(slug: string): IntelligenceItem | undefined {
  return intelligenceItems.find((item) => item.slug === slug);
}
